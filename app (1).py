...
st.set_page_config(page_title='StockMaster | Next', layout='wide')
...
<html lang="en">
    <head>
        ...
        <style>
            body { margin: 0; font-family: Arial, Helvetica, sans-serif; background: #eff6ff; color: #0f172a; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 2rem; }
            .card { background: #ffffff; border-radius: 24px; padding: 2rem 2.25rem; max-width: 640px; box-shadow: 0 20px 50px rgba(30, 64, 175, 0.12); text-align: center; }
            h1 { margin-bottom: 0.75rem; }
            p { line-height: 1.7; }
            .intro { margin-bottom: 1rem; color: #334155; }
            .form-grid { display: grid; gap: 1rem; margin-top: 1rem; text-align: left; }
            .field-row { display: grid; gap: 0.5rem; }
            .field-row label { font-weight: 700; color: #0f172a; }
            .field-row input, .field-row select { padding: 0.85rem 1rem; border-radius: 12px; border: 1px solid #cbd5e1; font-size: 1rem; font-family: inherit; }
            .actions { display: flex; gap: 0.75rem; justify-content: center; margin-top: 1rem; flex-wrap: wrap; }
            button { border: 0; border-radius: 999px; padding: 0.9rem 1.4rem; font-weight: 700; cursor: pointer; font-size: 1rem; }
            .primary-btn { background: #1d4ed8; color: #fff; }
            .secondary-btn { background: #e2e8f0; color: #0f172a; }
            .result-box { margin-top: 1rem; padding: 1rem; border-radius: 16px; background: #f8fafc; text-align: left; border: 1px solid #cbd5e1; }
            .result-box h2 { margin-top: 0; margin-bottom: 0.5rem; font-size: 1.1rem; }
            .result-box p { margin: 0.4rem 0; }
            .warning { color: #b45309; }
            .danger { color: #b91c1c; }
            .back { display: inline-block; margin-top: 1rem; color: #1d4ed8; font-weight: 700; text-decoration: none; }
        </style>
    </head>
    <body>
        <main class="card">
...
# Embed the full HTML into Streamlit's HTML component
st_html(HTML, height=900, scrolling=True)
